...
public partial class SaasKitContext : DbContext
{
    ...
    public virtual DbSet<MeteringLogs> MeteringLogs { get; set; }
    ...
    public virtual DbSet<SubscriptionUsers> SubscriptionUsers { get; set; }
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        modelBuilder.Entity<SubscriptionUsers>(entity =>
        {
            entity.HasOne(x => x.Subscription)
            .WithMany(x => x.SubscriptionUsers)
            .HasForeignKey(x => x.SubscriptionId);

            entity.Property(x => x.AmpSubscriptionId)
            .HasColumnName("AMPSubscriptionId")
            .IsRequired();
            ...
            entity
            .HasIndex(x => new { x.UserEmail, x.AmpSubscriptionId })
            .HasDatabaseName("IX_Email_AmpSubId")
            .IsUnique();
        });
        modelBuilder.Entity<MeteringLogs>(entity =>
        {
            entity
            .HasOne(x => x.Subscription)
            .WithMany(x => x.MeteringLogs)
            .HasForeignKey(x => x.SubscriptionId)
            .OnDelete(DeleteBehavior.NoAction);

            entity.HasOne(d => d.MeteredDimension)
                .WithMany(p => p.MeteringLogs)
                .HasForeignKey(d => d.DimensionId);

            entity.HasOne(x => x.SubscriptionUser)
                   .WithMany(x => x.MeteringLogs)
                   .HasForeignKey(x => x.SubscriptionUserId);
